...
(deftest defmulti-test
  (testing "defmulti creates multimethod"
    (is (eval* "(do (defmulti my-multi type) true)")))
  (testing "multimethod is callable"
    (is
     (eval*
      "(do (defmulti callable-multi identity)
...
(deftest isa-test
  (testing "isa? with equality" (is (true? (eval* "(isa? :foo :foo)"))))
  (testing "isa? with derived"
    (is (true? (eval* "(do (derive ::x ::y) (isa? ::x ::y))"))))
  (testing "isa? negative" (is (false? (eval* "(isa? :foo :bar)")))))
...
(deftest methods-test
  (testing "methods returns dispatch map"
    (is
     (eval*
      "(do (defmulti inspect-multi identity)
...
(deftest get-method-test
  (testing "get-method returns method fn"
    (is
     (eval*
      "(do (defmulti get-m-multi identity)
