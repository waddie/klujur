...
(deftest volatile-deref-test
  ...
  (testing "deref with nil value"
    (is (nil? (eval* "@(volatile! nil)")))))
...
(deftest volatile-characteristics-test
  ...
  (testing "atoms are not volatiles"
    (is (false? (eval* "(volatile? (atom 0))"))))
  (testing "volatiles work with deref"
    (is (= 42 (eval* "@(volatile! 42)")))))
...
(deftest volatile-use-cases-test
  (testing "volatile as counter in iteration"
    (is (= 5 (eval* "(let [counter (volatile! 0)]
                       (doseq [x [1 2 3 4 5]]
                         (vswap! counter inc))
                       @counter)"))))

  (testing "volatile as accumulator"
    (is (= [1 4 9 16] (eval* "(let [result (volatile! [])]
                                (doseq [x [1 2 3 4]]
                                  (vswap! result conj (* x x)))
                                @result)"))))

  (testing "volatile with conditional updates"
    (is (= [2 4] (eval* "(let [evens (volatile! [])]
                           (doseq [x [1 2 3 4 5]]
                             (when (even? x)
                               (vswap! evens conj x)))
                           @evens)")))))
...
(deftest volatiles-in-collections-test
  (testing "volatile stored in vector"
    (is (= 42 (eval* "(let [v (volatile! 42)
                             vec [v]]
                        @(first vec))"))))

  (testing "volatile stored in map"
    (is (= 100 (eval* "(let [v (volatile! 100)
                              m {:vol v}]
                         @(:vol m))"))))

  (testing "modifying volatile through collection"
    (is (= 11 (eval* "(let [v (volatile! 10)
                             vec [v]]
                        (vswap! (first vec) inc)
                        @v)")))))
...
(deftest vswap-complex-functions-test
  (testing "vswap! with anonymous function"
    (is (= 25 (eval* "(let [v (volatile! 5)]
                        (vswap! v (fn* [x] (* x x)))
                        @v)"))))

  (testing "vswap! with multi-arg function"
    (is (= {:a 1 :b 2} (eval* "(let [v (volatile! {})]
                                 (vswap! v assoc :a 1)
                                 (vswap! v assoc :b 2)
                                 @v)")))))
