...
(deftest defrecord-test
  (testing "defrecord creates record type"
    (is (eval* "(do (defrecord Point [x y]) true)")))
  (testing "positional constructor"
    (is
     (eval*
      "(do (defrecord Point2 [x y])
                     (let [p (->Point2 1 2)]
                       (= 1 (:x p))))")))
  (testing "map constructor"
    (is
     (eval*
      "(do (defrecord Point3 [x y])
  ...
  (testing "record implementing protocol"
    (is
     (=
      "Point(1, 2)"
      (eval*
       "(do (defprotocol IShow (show [x]))
                        (defrecord ShowPoint [x y])
                        (extend-type ShowPoint IShow
