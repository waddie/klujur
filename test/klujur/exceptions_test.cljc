...
(deftest catch-test
  (testing "catch catches exception"
    (is (= :caught
           (eval* "(try (throw \"error\") (catch Exception e :caught))"))))
  (testing "catch binds exception"
    (is (= "error" (eval* "(try (throw \"error\") (catch Exception e e))"))))
...
(deftest finally-test
  ...
  (testing "finally return value not used"
    (is (= 42 (eval* "(try 42 (finally :ignored))")))))
...
(deftest ex-info-test
  (testing "ex-info creates exception"
    (is (eval* "(map? (ex-info \"msg\" {:a 1}))")))
  (testing "ex-info with cause"
    (is (eval* "(map? (ex-info \"msg\" {:a 1} \"cause\"))"))))

(deftest ex-data-test
  (testing "ex-data returns map"
    (is (= {:reason :test}
           (eval* "(ex-data (ex-info \"msg\" {:reason :test}))"))))
  (testing "ex-data on non-ex-info"
    (is (nil? (eval* "(ex-data \"plain string\")")))))

(deftest ex-message-test
  (testing "ex-message returns message"
    (is (= "test message"
           (eval* "(ex-message (ex-info \"test message\" {}))"))))
  (testing "ex-message on string"
    (is (= "hello" (eval* "(ex-message \"hello\")")))))
